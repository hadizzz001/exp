 ...
 <div id="homepp" className="max-w-screen-xl mx-auto px-4">
      {exhibitionChunks.length > 0 ? (
        exhibitionChunks.map((chunk, index) => (
          <div
            key={index}
            className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 gap-1 mb-[3em] place-items-center"
          >
            {chunk.map((item, subIndex) => {
              ...
              // Use Tailwind CSS for responsive margin
              const marginClass = currentIndex % 2 === 0 ? 'sm:ml-20' : 'sm:mr-20';

              return (
                <div
                  key={subIndex}
                  className={`text-center ${marginClass} sm:mx-0`} // Apply margin based on even/odd index
                >
                  {!hideTitle && (
